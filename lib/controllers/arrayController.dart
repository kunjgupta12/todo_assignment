...
import 'package:todo_assignment/models/Array.dart';
...
class ArrayController extends GetxController {
  ...
  RxList<FTodo> urgent = RxList<FTodo>([]);
  RxList<Array> noUrgent = RxList<Array>([]);
  ...
  Stream<List<FTodo>> getUrgent() {
    return collectionReference
        .where('priority', isEqualTo: 'Urgent')
        .snapshots()
        .map((query) => query.docs.map((item) => FTodo.fromMap(item)).toList());
  }

  Stream<List<FTodo>> getAllTodos() {
    return allTodosCollectionReference
        .snapshots()
        .map((query) => query.docs.map((item) => FTodo.fromMap(item)).toList());
  }
